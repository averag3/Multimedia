...
function crearQR($rut, $digver){
	...
	foreach ($conn->query($sql) as $row) {
		$content = ($row['rut'].'-'.$row['digver'].' '.$row['fape'].' '.$row['sape'].' '.$row['fname'].' '.$row['sname'].' '.$row['address'].' '.$row['email']);
	}
...
function crearArchivo(){
	require 'connection.php';
	$sql = ("SELECT fname, sname, fape, sape, address, email, rut, digver FROM alumno");
	$lista_alumn = fopen("lista_alumno.txt", "w+") or die("Imposible de abrir archivo.");
	foreach ($conn->query($sql) as $row) {
		$txt = ($row['rut'].'-'.$row['digver'].' '.$row['fape'].' '.$row['sape'].' '.$row['fname'].' '.$row['sname'].' '.$row['address'].' '.$row['email']."\n");
		fwrite($lista_alumn, $txt);
	}
	fclose($lista_alumn);
}
...
if(isset($_REQUEST['crear_qr'])){
	$rut = $_REQUEST['rut'];
	$digver = $_REQUEST['digver'];
	crearQR($rut, $digver);
}

if(isset($_REQUEST['crear_lista'])){
	crearArchivo();
}
?>